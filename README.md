...
常用校验为 `just chk`（`nix flake check`）；`just update` 会先更新固定的 Reasonix CLI 包，再更新全部 flake inputs，单独更新 Reasonix 可运行 `just update-reasonix`。部署命令为 `just pi`、`just xps` 和 `just gtr7`。运行 `just --list` 可按 build、deploy、dev、maintenance、info 分组查看其他操作。输入源与具体版本以 `flake.nix`、`flake.lock` 为准。
