...
```text
fool/nvim/
├── default.nix       # fool.nvim.lsp/markdown option、外围包、programs.nixvim 入口
...
├── markdown.nix      # render-markdown + markdown/markdown_inline 解析器
...
- `default.nix`：
  - 声明 `fool.nvim.lsp` 与 `fool.nvim.markdown` option。
...
- `markdown.nix`：
  - 只在 `hmConfig.fool.nvim.markdown` 为真时启用。
  - 声明 `plugins.render-markdown` 及其样式（bullet/heading/code，取自插件 wiki）。
  - 声明本开关所需的 Treesitter grammar（`markdown`、`markdown_inline`）；`plugins.treesitter.enable` 用 `mkDefault`，与 `lsp.nix` 的 `enable = true` 不冲突，`grammarPackages` 两个模块合并为并集。

## 公开开关

- `fool.nvim.lsp`：启用 LSP server 与 Treesitter（仅桌面 profile 启用）。
- `fool.nvim.markdown`：启用 render-markdown（仅桌面 profile 启用），自带 markdown 解析器声明，可独立于 `fool.nvim.lsp` 打开。